...
const UserAdmin = () => {
  ...
  const emailResults = !searchTerm
    ...
    : Object.values(users)
        .filter((user) =>
          user.email.toLowerCase().startsWith(searchTerm.toLowerCase().trim())
        )
        .sort((a, b) => a.email.localeCompare(b.email))
        .map((u) => (
          <div onClick={userClickHandler(u)}>
  ...
  const updateUserDb = useCallback(async () => {
  ...
  }, [headerToSend, userManagedProjects, userToEdit._id]);
  ...
  }, [fetchUsers, updateUserDb, userLoaded, userManagedProjects]);
  ...
  const handleRemoveProject = (projectToRemove) => {
    if (userManagedProjects.length > 0) {
      setUserManagedProjects(
        userManagedProjects.filter((p) => p !== projectToRemove)
      );
    }
  };
  ...
  const toggleAddProject = () => {
    addNewProject === true ? setAddNewProject(false) : setAddNewProject(true);
  };
  ...
  if (
    ...
    return (
      <div className="container--usermanagement">
        <div>
          ...
          <div>
            <button className="button" onClick={handleProjectFormCancel}>
              Admin Dashboard
            </button>
          </div>
        </div>
    ...
    return (
      <div>
        <div>
          <button className="button" onClick={toggleUserSearch}>
            User Management
          </button>
        </div>
        <div>
          <button className="button" onClick={toggleAddProject}>
            Add New Project
          </button>
        </div>
      </div>
...
const EditUsers = (props) => {
  ...
  const userName =
    props.userToEdit.name?.firstName + ' ' + props.userToEdit.name?.lastName;
  ...
  const userProjectsToDisplay = prepareUserProjects(
    userProjects,
  ...
  return (
    <div>
              ...
              return (
                <li key={index}>
                  ...
                  <button
                    ...
                    onClick={() => props.handleRemoveProject(result[0])}
                  >
                    (remove)
                  </button>
                </li>
      ...
      <div>
          ...
          <form onSubmit={props.handleFormSubmit}>
            ...
            >
              ...
              {props.activeProjects.map((result, index) => {
                return (
                  <option key={index} value={result[0]}>
                    {result[1]}
                  </option>
                );
              })}
        ...
        <div>
          <button className="button-back" onClick={cancelEdit}>
            Back to search
          </button>
        </div>
        <div>
          <button className="button" onClick={props.handleFormCancel}>
            Admin Dashboard
          </button>
        </div>
      </div>
...
const AddNewProject = (props) => {
  ...
  const handleProjectFormSubmit = (event) => {
    ...
    const validationMatch = Object.values(props.projects)
      .filter(
        (project) =>
  ...
  return (
    <div className="add-new-project">
      ...
      <div>
        <button className="button-back" onClick={props.toggleAddProject}>
          Admin Dashboard
        </button>
      </div>
    </div>
