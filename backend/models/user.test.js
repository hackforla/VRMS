...
describe("Question Model saves the correct values", () => {
  test('Save a model instance and then read from the db', async (done) => {
    ...
    expect(savedData.name.firstName).toEqual(submittedData.name.firstName);
    expect(savedData.currentRole).toEqual(submittedData.currentRole);
    expect(savedData.desiredJobTitle).toEqual(submittedData.desiredJobTitle);
    done();
  });
